...
# ── Model Providers (UI selectable, all free-tier) ────────────────────────────
MODEL_PROVIDERS = {
    "Local (LM Studio)": {
    ...
    "Mistral – Small (Free)": {
        # Free tier via La Plateforme (mistral.ai)
...
# Detect Hugging Face Spaces environment (HF injects SPACE_ID automatically)
_IS_HF_SPACE = os.getenv("SPACE_ID") is not None

# Only expose providers that actually have a key configured.
# On HF Spaces, exclude Local (LM Studio) — it can't connect to a local server.
AVAILABLE_PROVIDERS = [
    name for name, cfg in MODEL_PROVIDERS.items()
    if cfg.get("api_key") and not (_IS_HF_SPACE and name.startswith("Local"))
]

# On HF Spaces, promote Mistral to first position so it's the default in the UI.
if _IS_HF_SPACE:
